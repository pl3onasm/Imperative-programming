...
void findSubsequence(char dna[], int nA, int nC, int nT, int nG, int subLen){
  ...
  for (int i=0; i<len; i++){
    ...
    if (i >= subLen-1){       
      ...
      if (a == nA && c == nC && t == nT && g == nG){
        if (!found) printf("%d", i-subLen+1);
        else printf(",%d", i-subLen+1);
        found = 1;
      }
      ...
      if (dna[i-subLen+1] == 'A') a--;
      if (dna[i-subLen+1] == 'C') c--;
      if (dna[i-subLen+1] == 'T') t--;
      if (dna[i-subLen+1] == 'G') g--;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  scanf("%s", dna);
  findSubsequence(dna, nA, nC, nT, nG, subLen);
